#include "LSM-OPD/sstable/RelFileManager.h"
...
namespace LSMOPD {
    ...
    VersionEdit *RelFileManager::MergeRelFile(RelCompaction<std::string> &compaction) {
        if (compaction.file_list.size() == 1)
        {
            ...
            std::string src_file_name = old_file->file_name;
            std::string dst_file_name = temp_file_metadata->file_name;
            ...
            edit->EditFileList.push_back(old_file);
        ...
        idx_t col_num = parsers[0]->GetColumnNum(); 
        ...
        std::string new_file_key_min = q.top().key;

        auto temp_file_metadata = new RelFileMetaData<std::string>(compaction.target_level, compaction.file_id, new_file_key_min, "", 0, 0);
        ...
        while (!q.empty()) {
            ...
            if ((size_t) key_buf_idx >= db->options->MEM_TABLE_MAX_SIZE) {
                ...
                if (!q.empty()) {
                    temp_file_metadata = new RelFileMetaData<std::string>(compaction.target_level, GetFileID(), q.top().key, "", 0, 0);
                    std::string file_name = temp_file_metadata->file_name;
