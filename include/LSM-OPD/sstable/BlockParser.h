...
namespace LSMOPD {
    ...
    public:
        ...
        BlockParser(BlockParser &&x): reader(x.reader), options(x.options), 
                                      offset_in_file(x.offset_in_file), block_size(x.block_size), 
                                      key_num(x.key_num), col_num(x.col_num), key_size(x.key_size), col_size(x.col_size),
                                      key_data_endpos(x.key_data_endpos), col_data_endpos(x.col_data_endpos), data_buffer(x.data_buffer)
                                      {
            x.valid = false;
        }
        ...
        void GetKeyCol(char *res) {
            char *buffer = data_buffer;
            std::memcpy(res, buffer, key_size * key_num);
        }
